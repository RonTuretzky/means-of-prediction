...
import qwen_final_audit as qa
ROOT=c.ROOT
SUPPORT=c.a.q.ROOT/'source-check-independent-review-v1/audit.py'
spec=importlib.util.spec_from_file_location('independent_measure',SUPPORT);support=importlib.util.module_from_spec(spec);spec.loader.exec_module(support)
...
qphase=qa.verify_phase('baseline','development')
...
for r in qr:
 ...
 x=support.strict(output) if valid else 'UNSCORABLE'
...
metrics={k:support.measure(labels,x) for k,x in ds.items()};assert metrics==comparison['methods']
